...
LPCSTR TargetProcess = "RainbowSix.exe";
bool ShowMenu = true;
...
void Draw() {
	RGBA color = {250,0,170, 255 };

	

	DrawStrokeText(30, 44, &color, "Possible Paste | v2.1 \n");

	DrawStrokeText(30, 60, &color, "[ Credits ]");
	DrawStrokeText(30, 70, &color, "[+] Paging");
	DrawStrokeText(30, 80, &color, "[+] Possible TM");
...
void Render() {
	...
	if (ShowMenu == true) {
		InputHandler();
		ImGui::Begin("Possible Paste | v2.1");
...
DWORD WINAPI ProcessCheck(LPVOID lpParameter) {
	while (true) {
		if (Process.Hwnd != NULL) {
			if (find_process_by_id(TargetProcess) == 0) {
				exit(0);
			}
		}
	}
...
int main() {
	...
	while (WindowFocus == false) {
		...
		if (find_process_by_id(TargetProcess) == ForegroundWindowProcessID) {
